
	<article class="site-article layout-homepage">
		
		<section class="section-style1 section-values">
		
			<div class="container-fixed container-fixed-narrow">
			
				<h2 class="text-center">ICOR įmonių <s></s>grupės skaičiai</h2>
			
				<ul class="list-unstyled row">
					
					<li class="text-center col-xs-12 col-md-4">
						
						<span class="row">
							<span class="col-xs-6 col-md-12">
								<span class="center-block"><span class="list-icon img-icon-apyvarta"></span></span>
							</span>
							<span class="col-xs-6 col-md-12">
								<strong class="center-block">Apyvarta</strong>
								<span class="center-block">
									<span class="trow trow-vmiddle">
										<span class="text-primary text-value">450</span>
										<span class="text-primary text-unit">mln<br />eur</span>
									</span>
								</span>
							</span>
						</span>
						
					</li>
					
					<li class="text-center col-xs-12 col-md-4">
						<span class="row">
							<span class="col-xs-6 col-md-12">
								<span class="center-block"><span class="list-icon img-icon-darbuotojai"></span></span>
							</span>
							<span class="col-xs-6 col-md-12">
								<strong class="center-block">Darbuotojai</strong>
								<span class="center-block"><span class="text-primary text-value">6 400</span></span>
							</span>
						</span>
					</li>
					
					<li class="text-center col-xs-12 col-md-4">
						<span class="row">
							<span class="col-xs-6 col-md-12">
								<span class="center-block"><span class="list-icon img-icon-geografija"></span></span>
							</span>
							<span class="col-xs-6 col-md-12">
								<strong class="center-block">Veiklos geografija</strong>
								<span class="center-block">
									<span class="trow trow-vmiddle">
										<span class="text-primary text-value">40+</span>
										<span class="text-primary text-unit">šalių</span>
									</span>
								</span>
							</span>
						</span>
					</li>
					
				</ul>
				
			</div>
		
		</section>
		
		
		<section class="section-style2">
		
			<div class="container-fixed container-fixed-narrow">
		
				<p class="text-center text-h3">Savo veiklą pradėjusi 1990 metais, šiandien „Icor“ yra viena didžiausių įmonių grupių regione. Grupės įmonės sėkmingai dirba komunalinių paslaugų, pramonės ir energetikos, prekybos naftos produktais, nekilnojamo turto plėtros ir pramogų srityse.</p>
		
			</div>
			
		</section>
		
		
		
		<section class="section-style3 section-companies">
		
			<div class="container-fixed">
		
				<h2>ICOR <s></s>veiklos sritys</h2>
				
				<p>Įmonių grupei „ICOR“ priklauso apie 70 įmonių, kurios dirba keturiose pagrindinėse veiklos srityse. Tai:</p>
				
				<ul class="list-unstyled row">
					
					<li class="text-center col-xs-12 col-sm-6 col-md-3">
						<span class="center-block"><a class="list-icon img-icon-cityservice" href="http://www.lrytas.lt/"></a></span>
						<strong class="center-block"><a href="/">Komunalinės paslaugos</a></strong>
						<span class="center-block text-muted"><a href="/">„City Service“</a></span>
					</li>
					
					<li class="text-center col-xs-12 col-sm-6 col-md-3">
						<span class="center-block"><a class="list-icon img-icon-axioma" href="http://www.lrytas.lt/"></a></span>
						<strong class="center-block"><a href="/">Pramonė ir energetika</a></strong>
						<span class="center-block text-muted"><a href="/">„Axis Industries“</a></span>
					</li>
					
					<li class="text-center col-xs-12 col-sm-6 col-md-3">
						<span class="center-block"><a class="list-icon img-icon-realco" href="http://www.lrytas.lt/"></a></span>
						<strong class="center-block"><a href="/">Nekilnojamasis turtas</a></strong>
						<span class="center-block text-muted"><a href="/">„Realco“</a></span>
					</li>
					
					<li class="text-center col-xs-12 col-sm-6 col-md-3">
						<span class="center-block"><a class="list-icon img-icon-oilead" href="http://www.lrytas.lt/"></a></span>
						<strong class="center-block"><a href="/">Nafta</a></strong>
						<span class="center-block text-muted"><a href="/">OiLead</a></span>
					</li>
					
				</ul>
			
			</div>
			
		</section>
		
		
		
		<section class="section-style1">
		
			<div class="container-fixed">
			
				<h2>ICOR veiklos <s></s>geograﬁja</h2>
				
				<p>Lorem ipsum dolor sit amet, consectetuer adipiscing elit, sed diam nonummy nibh euismod tincidunt ut laoreet dolore magna aliquam erat </p>
				
				<div class="map-image">
					<img class="img-responsive" src="/images/map_1x.jpg" srcset="/images/map_1x.jpg 1x, /images/map_2x.jpg 2x" alt="" />
				</div>
				
			</div>
			
		</section>
		
		
		
		<section class="section-style3 section-footer">
		
			<div class="container-fixed">
				
				<div class="row">
				
					<div class="col-xs-12 col-md-6">
						
						<p><span class="img-icon-icor-footer"></span></p>
						
						<p>
							<strong>UAB „ICOR“<span class="sep">,</span></strong> Konstitucijos pr. 7, 28 a., LT-09308 Vilnius<br />
							Tel. <a href="tel:+37052394800">(8 5) 239 4800</a>, faks. (8 5) 239 4901, el. paštas 
							<script type="text/javascript">
								document.write('<a'+' hre'+'f="m'+'ailto'+':in'+'fo@ic'+'or.lt">in'+'fo@i'+'cor.lt</a>');
							</script>
						</p>
						
					</div>
					
					<div class="col-xs-12 col-md-6">
				
						<p>
							<strong>Įmonės rekvizitai</strong><br />
							UAB „ICOR“<br />
							Įmonės kodas 300021944<br />
							PVM mokėtojo kodas LT100001025611<br />
							AB DNB bankas<br />
							Banko kodas 40100<br />
							A. s. LT334010042400916337<br />
							Duomenys apie juridinį asmenį kaupiami ir saugomi LR juridinių asmenų registre, registro tvarkytojas – VĮ „Registrų centras“.
						</p>
					
					</div>
					
				</div>
					
			</div>
			
		</section>
		
		
	
	</article>
